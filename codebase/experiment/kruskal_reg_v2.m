function [beta0_final,beta_final,glmstats_final,dev_final, dev0_avg, iter_avg] = ...
    kruskal_reg_v2(X,M,y,r,dist,varargin)
% KRUSKAL_REG_V2 Fit the rank-r Kruskal tensor regression
%   [BETA0,BETA,GLMSTATS,DEV,DEV0,ITER] = KRUSKAL_REG(X,M,Y,R,DIST) fits the Kruskal
...
%   summary statistics of GLM fit from the last iteration, DEV is the
%   deviance of final model, DEV0 is the average deviance of initial values
%   and ITER is the average iteration number of replicates.
%
%   [BETA0,BETA,GLMSTATS,DEV,DEV0,ITER] =
...
%       dev0_avg: average deviance of initial values
%       iter_avg: average iteration number of replicates
%
% Examples
%
...
%
% What's new
%    - add a pruning strategy to avoid useless iterations
%
% Reference
...
for rep=1:Replicates   
    ...
    [~, devtmp] = glmfit_priv([X, double(ttt(TM, tensor(beta), 1:ndims(beta)))], y, dist, 'constant', 'off', 'weights', wts);
    dev0_avg = dev0_avg + devtmp;
    ...
    for iter=1:MaxIter
        ...
        if (iter==1)
            ...
            % ===============TRIAL==================
            % pruning rule
            if diffdev * (MaxIter-iter) < 0.1 * (devtmp-dev_final)                
                break;
            end
            % ====================================
...
iter_avg = iter_avg / Replicates;
dev0_avg = dev0_avg / Replicates;
